...
router.get("/dashboard", requireAuth, async (req, res, next) => {
  try {
    ...
    const profileRows = await db.query(
      `SELECT
          user_id, display_name, email, email_verified, account_type, created_at,
          bio, headline, location, avatar_initial, privacy_level, avg_rating, rating_count,
          (SELECT COUNT(*) FROM skill_test_attempts sta WHERE sta.user_id = u.user_id AND sta.passed = 1) AS verified_badges
       FROM users u
    ...
    const mySkills = await db.query(
      `SELECT s.skill_id, s.title, s.description, s.skill_type, s.mode, s.is_active,
              c.name AS category_name,
              u.avatar_initial, u.headline, u.avg_rating, u.rating_count,
              o.org_name, tag_map.tags,
              EXISTS (SELECT 1 FROM skill_test_attempts sta WHERE sta.skill_id = s.skill_id AND sta.user_id = s.owner_user_id AND sta.passed = 1) AS verified_badge
       FROM skills s
       ...
       LEFT JOIN (
         ...
         FROM skill_tag_map stm JOIN skill_tags st ON st.tag_id = stm.tag_id GROUP BY stm.skill_id
       ) AS tag_map ON tag_map.skill_id = s.skill_id
       WHERE s.owner_user_id = ? ORDER BY s.updated_at DESC`,
      [userId]
    );

    const myExchanges = await db.query(
      `SELECT e.exchange_id, e.status, e.created_at, s.title AS skill_title,
              CASE WHEN e.requester_id = ? THEN provider.display_name ELSE requester.display_name END AS counterpart_name
    ...
    const memberships = await db.query(
      `SELECT o.org_name, m.role FROM memberships m
       JOIN organisations o ON o.org_id = m.org_id
       WHERE m.user_id = ? AND m.is_active = 1 ORDER BY o.org_name ASC`,
      [userId]
    );
    ...
    if (req.session.user.is_admin) {
      adminReports = await db.query(
        `SELECT r.report_id, r.target_type, r.reason, r.notes, r.status, r.created_at,
         ...
         WHERE r.status IN ('OPEN','IN_REVIEW') ORDER BY r.created_at DESC`
      );
    }
...
router.get("/edit", requireAuth, async (req, res, next) => {
  try {
    const userId = req.session.user.user_id;
    const profileRows = await db.query(
      `SELECT display_name, bio, headline, location, avatar_initial, privacy_level
...
router.put("/edit", requireAuth, async (req, res, next) => {
  try {
    ...
    await db.query(
      `UPDATE users SET bio = ?, headline = ?, location = ?, avatar_initial = ?, privacy_level = ?
       WHERE user_id = ?`,
      [bio, headline, location, avatarInitial,
       ["PUBLIC", "REGISTERED", "PRIVATE"].includes(privacyLevel) ? privacyLevel : "PUBLIC",
       userId]
    );

    await createNotification(userId, "PROFILE_UPDATED", "Your profile was updated.");
...
router.post("/:userId/report", requireAuth, async (req, res, next) => {
  try {
    ...
    await db.query(
      `INSERT INTO reports (report_id, created_by_user_id, target_type, target_user_id, reason, notes, evidence_url, status)
       VALUES (?, ?, 'USER', ?, ?, ?, ?, 'OPEN')`,
      [crypto.randomUUID(), req.session.user.user_id, req.params.userId, reason, notes, evidenceUrl]
    );
    await createNotification(req.session.user.user_id, "REPORT_CREATED", "Your user report has been sent to moderation.");
...
router.get("/:userId", async (req, res, next) => {
  try {
    const userRows = await db.query(
      `SELECT user_id, display_name, email_verified, bio, headline, location,
              avatar_initial, privacy_level, avg_rating, rating_count,
              (SELECT COUNT(*) FROM skill_test_attempts sta WHERE sta.user_id = u.user_id AND sta.passed = 1) AS verified_badges
       FROM users u WHERE user_id = ? LIMIT 1`,
      [req.params.userId]
    );
    if (!userRows.length) {
      return res.status(404).render("pages/404", { title: "Profile not found", pageClass: "page-404" });
    }
    ...
    const skills = canViewFull ? await db.query(
      `SELECT s.skill_id, s.title, s.description, s.skill_type, s.mode,
              c.name AS category_name, u.avatar_initial, u.headline, u.avg_rating, u.rating_count,
              tag_map.tags,
              EXISTS (SELECT 1 FROM skill_test_attempts sta WHERE sta.skill_id = s.skill_id AND sta.user_id = s.owner_user_id AND sta.passed = 1) AS verified_badge
       FROM skills s
       LEFT JOIN categories c ON c.category_id = s.category_id
       ...
       WHERE s.owner_user_id = ? AND s.is_active = 1 ORDER BY s.updated_at DESC`,
      [profile.user_id]
    ) : [];

    const reviews = canViewFull ? await db.query(
      `SELECT r.score, r.comment, r.created_at, u.display_name AS rater_name
       FROM ratings r JOIN users u ON u.user_id = r.rater_id
       WHERE r.rated_id = ? ORDER BY r.created_at DESC LIMIT 8`,
      [profile.user_id]
    ) : [];
